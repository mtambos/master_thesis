...
def iterate_images(image_paths=None, image_arrays=None):
    assert ((image_paths is None and isinstance(image_arrays, Iterable)) or
            (isinstance(image_paths, Iterable) and image_arrays is None))
    if image_arrays is not None:
        for img in image_arrays:
            yield img
    if image_paths is not None:
        for im_path in image_paths:
            yield sp_imread(im_path, mode='RGB')


def preprocess_image_batch(image_paths=None, image_arrays=None, img_size=None,
                           crop_size=None, color_mode="rgb", out=None):
    img_list = []

    for img in iterate_images(image_paths, image_arrays):
        if img_size:
