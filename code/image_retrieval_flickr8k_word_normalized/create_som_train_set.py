...
def create_nouns_df(nouns_df_fname, captions_df):
    ...
    if nouns_df_fname.exists():
        ...
        nouns_vectors = []
        nouns = []
        for i, d in enumerate(nlp.pipe(tqdm(captions_df.doc), batch_size=1000, n_threads=8)):
            for t in d:
                if t.pos_ == 'NOUN' or t.pos_ == 'PROPN':
                    nouns_vectors.append(t.vector)
                    nouns.append(str(t))
        nouns_df = pd.DataFrame(
            nouns_vectors, columns=[f'vec_{i}' for i in range(300)]
        )
        nouns_df['word'] = nouns
...
def generate_batches(captions, img_dir, batch_size=1):
    ...
    for fname, noun in tqdm(nouns, desc="fname-noun"):
        img_features = get_image_features(fname)[0]
        img_features /= np.linalg.norm(img_features, ord=2)
        noun_vector = noun.vector / np.linalg.norm(noun.vector, ord=2)
        X[i] = np.r_[img_features, noun_vector]
        fnames[i] = fname
        i += 1
        if i == batch_size:
            i = 0
...
def main(batch_size, img_dir, tokens_file, save_to_dir, build_nouns_df):
    ...
    if build_nouns_df:
        click.echo('Creating nouns.pickle')
